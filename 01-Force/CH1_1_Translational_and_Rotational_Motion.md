...
```mermaid
graph TD
    A["⚙️ FORCE applied to a RIGID BODY"]
    A --> B{"Is the body\nFIXED at any point?"}
    B -->|"NO — Completely Free"| C["🏃 TRANSLATIONAL MOTION\nEvery particle: same velocity\nSame direction, same speed"]
    B -->|"YES — Has a Pivot/Hinge"| D["🔄 ROTATIONAL MOTION\nEvery particle: different speed\nAll tracing circular arcs\naround the pivot"]
    B -->|"BOTH conditions"| E["🌀 COMBINED MOTION\n(e.g. Rolling Wheel)\nTranslation + Rotation simultaneously"]

    style A fill:#1a1a2e,color:#eee,stroke:#7c83fd
    style B fill:#16213e,color:#eee,stroke:#7c83fd
    style C fill:#0f3460,color:#eee,stroke:#00b4d8
    style D fill:#533483,color:#eee,stroke:#e94560
