...
```mermaid
graph TD
    A["⚙️ A COUPLE in Action"]
    A --> B["🚗 Steering Wheel\nBoth hands push opposite sides\nd = wheel diameter"]
    A --> C["🔑 Turning a Key\nThumb + finger\nd = key width"]
    A --> D["🪛 Screwdriver Handle\nWide grip = large d"]
    A --> E["🚰 Water Tap\nFinger + thumb\nd = tap diameter"]
    style A fill:#1a1a2e,color:#eee,stroke:#7c83fd
    style B fill:#16213e,color:#eee,stroke:#00b4d8
    style C fill:#0f3460,color:#eee,stroke:#52b788
